...
def run_loop(args):
    ...
    if restore:
        with open('./StochEnv-v2/09-15-03-55/replay_buffer_21499.pkl', 'rb') as handle:
    ...
    if restore:
        sac_policy.ac.load_state_dict(torch.load('./StochEnv-v2/09-15-03-55/sac_policy_21499.pt', map_location=torch.device('cuda')))
        dynamics.model.ensemble_model.load_state_dict(torch.load('./StochEnv-v2/09-15-03-55/dynamics_21499.pt', map_location=torch.device('cuda')))
        dynamics.model.elite_model_idxes, dynamics.model.scaler = pickle.load(open('./StochEnv-v2/09-15-03-55/scalar_trasform_21499.pkl', 'rb'))
    else:
        ckpt = 0
