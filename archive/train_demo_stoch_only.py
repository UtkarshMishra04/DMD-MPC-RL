...
def run_loop(args):
    ...
    logger_kwargs={'output_dir':args.exp_name, 'exp_name':datenow.strftime("%m-%d-%H-%M")}
    ...
    if restore:
        with open('./StochSAC-v2/09-15-01-31/replay_buffer_5999.pkl', 'rb') as handle:
            ...
            print(ckpt, replay_buffer.state[ckpt-100])
    ...
    if restore:
        sac_policy.ac.load_state_dict(torch.load('./StochSAC-v2/09-15-01-31/sac_policy_5999.pt', map_location=torch.device('cuda')))
        dynamics.model.ensemble_model.load_state_dict(torch.load('./StochSAC-v2/09-15-01-31/dynamics_5999.pt', map_location=torch.device('cuda')))
        dynamics.model.elite_model_idxes, dynamics.model.scaler = pickle.load(open('./StochSAC-v2/09-15-01-31/scalar_trasform_5999.pkl', 'rb'))
        sac_policy.ac_targ = deepcopy(sac_policy.ac)
    else:
        ckpt = 0
    ...
    for t in tqdm(range(start_instant, int(args.max_timesteps))):
        ...
        action = sac_policy.get_action(np.array(state))
        ...
        if (t + 1) % args.eval_freq == 0 and t > args.start_timesteps:
            ...
            logger.log('eval/actor_eval', actor_rew, t)
            logger.log('eval/episode_reward', evaluation_rewards, t)
